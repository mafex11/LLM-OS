...
class WindowsUseGUI:
    ...
    def show_settings(self):
        ...
        # Adaptive detection
        adaptive_var = tk.BooleanVar(value=self.agent.desktop.use_adaptive_detection)
        ttk.Checkbutton(settings_frame, text="Enable Adaptive Detection", 
                       variable=adaptive_var, 
                       command=lambda: self.agent.desktop.set_adaptive_detection(adaptive_var.get())).pack(anchor=tk.W, pady=2)
        
        # Intelligent detection
        intelligent_var = tk.BooleanVar(value=self.agent.desktop.use_intelligent_detection)
        ttk.Checkbutton(settings_frame, text="Enable Intelligent Detection", 
                       variable=intelligent_var,
                       command=lambda: self.agent.desktop.set_intelligent_detection(intelligent_var.get())).pack(anchor=tk.W, pady=2)

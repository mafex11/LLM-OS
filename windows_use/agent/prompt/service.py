...
class Prompt:
    @staticmethod
    def system_prompt(browser: str,language: str,tools_prompt:str,max_steps:int,instructions: list[str]=[],literal_mode:bool=True) -> str:
        width, height = pg.size()
        template =PromptTemplate.from_file(files('windows_use.agent.prompt').joinpath('system.md'))
        # Add literal mode instructions if enabled
        literal_instructions = []
        if literal_mode:
            literal_instructions = [
                "LITERAL MODE ENABLED: Only perform exactly what the user asks for.",
                "Do NOT add extra steps like saving, closing, or completing tasks that weren't explicitly requested.",
                "Wait for the user's next instruction after completing the requested action."
            ]
        
        all_instructions = instructions + literal_instructions
